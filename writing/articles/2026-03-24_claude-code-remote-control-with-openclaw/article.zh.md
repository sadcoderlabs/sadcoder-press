...
整個設定過程不到十分鐘。但接下來發生的事讓我重新想了一些事情。
...
Wharton 商學院教授 Ethan Mollick 在 Dispatch 上線後說，[Dispatch 已經能滿足 90% 的 OpenClaw 需求](https://causalinf.substack.com/p/claude-code-34-using-dispatch-on)。這句話從學界傳出來，代表業界確實在用 OpenClaw 作為通用 agent 的標竿。
...
這些問題有一個共同的特徵：它們都是「還沒做」，不是「做不到」。功能上有取捨，但小細節最後都做得到。
...
我做的事情很簡單。一份 `CLAUDE.md`，一個 skills 的 symlink，加上一行啟動指令。這樣就能讓 Claude Code 的行為逼近 OpenClaw。這代表兩者之間的距離，比表面看起來的短很多。

問題從來不是 OpenClaw 怎麼提供價值。問題是模型廠商有沒有想把重點放在這個領域。這個領域對他們來說，是不是只要再多做一點點就可以拿下的市場？
