...
autoDream 以 forked subagent 在背景跑，做四件事：掃描現有記憶建立地圖，收集新訊號找出跟程式碼對不上的「漂移記憶」，把整合結果寫入 topic files（刪矛盾、合併重疊、日期從相對轉絕對），最後修剪 MEMORY.md 索引到 200 行、25KB 以內。
...
我們在 OpenClaw 也做記憶整合，但方向不同。Claude Code 的記憶整理是為了省 token、提升效率。我們想探索的是：這條記憶重要嗎？為什麼重要？最終讓 agent 從記憶裡長出偏好和觀點。
